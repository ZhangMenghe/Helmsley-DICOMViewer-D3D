...
void viewAlignedSlicingRenderer::updateVertices(ID3D11DeviceContext* context, glm::mat4 model_mat){
	...
	for (int i = 0; i < 8; i++) {
		glm::vec4 v(cube_vertices[3 * i], cube_vertices[3 * i + 1], cube_vertices[3 * i + 2], 1.0f);
	...
	glm::vec3 pp = vec3(cube_vertices[id], cube_vertices[id + 1], cube_vertices[id + 2]);
	glm::vec3 pp_end = vec3(cube_vertices[end_id], cube_vertices[end_id + 1], cube_vertices[end_id + 2]);
