...
void paintCanvas::onBrushDraw(ID3D11DeviceContext* context, float px, float py) {
	...
	if (m_interpolate_sparse
		...
		//auto bleft = std::max(0, int(std::min(m_tex_u, curr_tex_u) - m_brush_radius));
		//auto bright = std::min(m_pw, UINT(std::max(m_tex_u, curr_tex_u) + m_brush_radius));
		//auto btop = std::max(0, int(std::min(m_tex_v, curr_tex_v) - m_brush_radius));
		//auto bbottom = std::min(m_ph, UINT(std::max(m_tex_v, curr_tex_v) + m_brush_radius));
		
		//auto width = bright - bleft;
		//auto height = bbottom - btop;

		//float slope = float(height) / width;
		//sample x
		//int x0 = std::max(0, int(std::min(m_tex_u, curr_tex_u))), y0 = std::min(m_ph, UINT(std::max(m_tex_v, curr_tex_v)));
		//int x1 = std::min(m_pw, UINT(std::max(m_tex_u, curr_tex_u))), y1 = std::max(0, int(std::min(m_tex_v, curr_tex_v)));
