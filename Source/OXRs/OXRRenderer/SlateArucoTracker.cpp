...
void SlateArucoTracker::FrameReadyCallback(IResearchModeSensorFrame* pSensorFrame, PVOID frameCtx){
    SlateArucoTracker* pSlateTexture = (SlateArucoTracker*)frameCtx;
...
void SlateArucoTracker::UpdateSlateTextureWithBitmap(const BYTE *pImage, UINT uWidth, UINT uHeight){
    // update m_texture2D
...
void SlateArucoTracker::UpdateSlateTexture(){
    HRESULT hr = S_OK;
...
//void SlateArucoTracker::EnsureSlateTexture()
//{
...
void SlateArucoTracker::FrameProcessing(){
    HRESULT hr = S_OK;
...
void SlateArucoTracker::StartCVProcessing(BYTE bright){
    m_Width = 200;
    m_Height = 200;
    //m_texture2D = nullptr;
