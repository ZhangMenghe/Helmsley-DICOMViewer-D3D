...
const int RGBFrameProcessor::kImageWidth = 760;
const wchar_t RGBFrameProcessor::kSensorName[3] = L"PV";
int RGBFrameProcessor::frame_count = 0;
winrt::Windows::Foundation::IAsyncAction RGBFrameProcessor::InitializeAsync(){
    ...
    m_OnFrameArrivedRegistration = mediaFrameReader.FrameArrived({ this, &RGBFrameProcessor::OnFrameArrived });
}

void RGBFrameProcessor::OnFrameArrived(const MediaFrameReader& sender, const MediaFrameArrivedEventArgs& args)
{
...
void RGBFrameProcessor::Clear()
{
    std::lock_guard<std::shared_mutex> lock(m_frameMutex);
    m_PVFrameLog.clear();
}

void RGBFrameProcessor::AddLogFrame()
{
...
void RGBFrameProcessor::DumpFrame(const SoftwareBitmap& softwareBitmap, long long timestamp){
    ...
    //m_FrameMat = cv::Mat(softwareBitmap.PixelHeight(), softwareBitmap.PixelWidth(),
    //    CV_8UC4, // assume input SoftwareBitmap is BGRA8
    //    (void*)pixelBufferData);
    //std::cout << "height " << softwareBitmap.PixelHeight() << " " << softwareBitmap.PixelWidth() << std::endl;
...
bool RGBFrameProcessor::DumpDataToDisk(const StorageFolder& folder, const std::wstring& datetime_path)
{
...
void RGBFrameProcessor::StartRecording(const StorageFolder& storageFolder, const SpatialCoordinateSystem& worldCoordSystem)
{
...
void RGBFrameProcessor::StopRecording()
{
    std::lock_guard<std::mutex> guard(m_storageMutex);
    //m_tarball.reset();
...
void RGBFrameProcessor::CameraWriteThread(RGBFrameProcessor* pProcessor){
