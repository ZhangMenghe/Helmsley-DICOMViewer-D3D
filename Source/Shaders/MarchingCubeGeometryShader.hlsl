...
Buffer<int> const_edge_table  : register(t2);
...
void main(uint3 threadID : SV_DispatchThreadID){
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		if (triangle_table[triangle_start_memory + 3 * i] != -1){
			Vertex v0 = vertex_list[triangle_table[triangle_start_memory + 3 * i]];

			output_vertices[idx].position = v0.position;
			output_vertices[idx].normal = v0.normal;

			Vertex v1 = vertex_list[triangle_table[triangle_start_memory + 3 * i + 1]];
			output_vertices[idx + 1].position = v1.position;
			output_vertices[idx + 1].normal = v1.normal;

			Vertex v2 = vertex_list[triangle_table[triangle_start_memory + 3 * i + 2]];
